...
export function BackButton({
  ...
  const isConnectionStatusCollapsed = useConnectionStatusUi.use.isCollapsed();
  const connectionStatusTone = useConnectionStatusUi.use.tone();
  const { t } = useI18n();
  const destination =
    targetTab === 'timer'
  ...
  return (
    ...
    >
      {isConnectionStatusCollapsed ? (
        <button
          type="button"
          onClick={handleBack}
          aria-label={destination}
          data-testid="connection-status-collapsed"
          title={`${t('connection.connecting')}. ${destination}`}
          className={`${
            className ||
            'flex items-center gap-1.5 text-sm text-slate-400 transition-colors'
          } cursor-pointer ${
