...
func makeNotificationContent(_ notification: Notification) throws -> UNNotificationContent {
  ...
  if let sound = notification.sound {
    content.sound = makeNotificationSound(sound)
  }
...
func makeNotificationSound(_ sound: String) -> UNNotificationSound {
  let fileName = URL(fileURLWithPath: sound).lastPathComponent
  ...
  let ext = path.pathExtension.isEmpty ? nil : path.pathExtension
