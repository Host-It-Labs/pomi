...
#[command]
pub(crate) async fn get_android_foreground_sync_status<R: Runtime>(
    _app: AppHandle<R>,
    notification: State<'_, Notifications<R>>,
) -> Result<serde_json::Value> {
    notification.get_android_foreground_sync_status().await
}
...
pub(crate) async fn start_android_foreground_sync<R: Runtime>(
    ...
    notification.start_android_foreground_sync().await
}

#[command]
pub(crate) async fn stop_android_foreground_sync<R: Runtime>(
    _app: AppHandle<R>,
    notification: State<'_, Notifications<R>>,
    clear_opt_in: bool,
    clear_auth: bool,
    ...
    notification
        .stop_android_foreground_sync(clear_opt_in, clear_auth)
        .await
}
