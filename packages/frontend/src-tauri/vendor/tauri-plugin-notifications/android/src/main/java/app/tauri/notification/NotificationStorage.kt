...
class NotificationStorage(private val context: Context, private val jsonMapper: ObjectMapper) {
  ...
  private fun parseNotification(json: String?): Notification? {
    ...
    return try {
      val notificationJson = JSONObject(json)
      val extra = notificationJson.optJSONObject("extra")
      notificationJson.remove("extra")

      val notification = jsonMapper.readValue(notificationJson.toString(), Notification::class.java)
      if (extra != null) {
        notification.extra = JSObject.fromJSONObject(extra)
      }
      notification
    } catch (e: Exception) {
