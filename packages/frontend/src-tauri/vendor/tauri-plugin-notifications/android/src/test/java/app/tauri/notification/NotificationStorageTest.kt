...
class NotificationStorageTest {
    ...
    @Test
    fun testGetSavedNotification_withExtraPayload() {
        every { mockSharedPreferences.getString("1", null) } returns """
            {
              "id": 1,
              "title": "Timer done",
        ...
        assertEquals("Timer done", result?.title)
        assertEquals("timer-1", result?.extra?.getString("timerId"))
        assertEquals("complete", result?.extra?.getString("notificationType"))
    }
