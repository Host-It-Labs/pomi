...
- Copyme enables all safe in-app capabilities, including Tasks during breaks, minimized Tasks, extension, undo, Assistant, transcript retention, and administrator capture logging. Contradictory auto-starts and OS/global side effects remain off. No overdue-grace preference is seeded.
- Fixture data is a representative matrix across Timer types, Task states/priorities/due states/recurrence/reminders/events, Parent/Sub-intentions, durations, statistics, and two clearly synthetic capture diagnostics. It does not seed runtime Timer/Session Redis state, secrets, models, budgets, raw audio, or provider envelopes.
