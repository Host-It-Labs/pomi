...
The `PR E2E` workflow runs once for each pushed commit. A manual workflow dispatch with `acceptance_timing` enabled runs the measured 10+3 sequence and uploads `e2e-acceptance-timing`; failures upload the partial timing report with the ordinary diagnostics.
