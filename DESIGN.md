...
- Embed the full task workspace beneath the timer. Desktop shows five rows per page, each with a two-line title; Up/Down changes pages even from workspace inputs, while open pickers retain option navigation and dialogs remain isolated. Desktop clips task overflow without allowing scrolling; page changes fade without positional animation. Mobile scrolls task rows independently.
...
- `Mod+T` expands the app and focuses quick creation. `Mod+N` also focuses quick creation; the full editor is opened through its explicit button. `Mod+Shift+F` clears search, filters, selected List, and sort back to the normal list. Repeated focus shortcuts keep the targeted input focused or its picker open. Escape blurs those inputs and closes dropdowns without clearing drafts. Command/Ctrl shortcuts remain available from main-page inputs; ordinary typing, including Space, stays in the input. Shortcut actions must respect open editors.
...
You might like shows up to two relevant disabled tools in stable order. Provide a feature-specific icon, benefit, enable/setup action, and Not interested. Confirm Not interested in a modal explaining that the setting remains available below and through Settings search. Dismissals persist per account across devices and export/import; they never disable a feature or remove its searchable control. AI infrastructure and capture diagnostics live on a separate admin-only Admin page linked from General. The page keeps AI administration distinct from an opt-in Capture logs section. Capture logs retain only the signed-in administrator's submitted AI Task capture input, allowlisted normalized output, outcome, and stage timings; disabling logging deletes all of that administrator's logs. The link, search entries, reads, exports, flags, clears, and writes are admin-only in every environment. Ordinary Assistant preferences remain separate.
